...
class TutoringSystem:
    ...
    def generate_id(self, entity_type: str) -> str:
        """ Generate consistent IDs for all entity types entity_type: 'student', 'tutor', 'session', or 'request'"""
        prefix_map = {
        ...
        try:
            ...
            # Verify length
            max_length = {
                'st_': 6, 'ttr_': 7, 'sess_': 8, 'req_': 7
            }[prefix]

            if len(new_id) > max_length:
                raise ValueError(f"Generated ID {new_id} exceeds maximum length of {max_length}")

            return new_id

        except Error as e:
            print(f"Error generating {entity_type} ID: {e}")
            raise
    ...
    def register_user(self, role: str) -> Optional[str]:
        ...
        try:
            ...
            # Create new account
            if role == 'student':
                user_id = self.generate_id('student')
                cursor.execute(
                    "INSERT INTO students (student_id, name, email) VALUES (%s, %s, %s)",
                    (user_id, name, email)
                )
            else:  # tutor
                user_id = self.generate_id('tutor')
                cursor.execute(
    ...
    def login_user(self) -> bool:
        ...
        user_id = input("Enter your ID (student_XXX for student, tutor_XXX for tutor): ").strip()
        ...
        if sub_choice == '1':
            ...
            try:
                ...
                if existing_request:
                    ...
                    # Create new request
                    request_id = self.generate_id('request')
                    cursor.execute('''
